...
last_update <- function(dl = deadline+1) {
  last = Sys.time()
  if (last>dl)
    last = dl
  format(last, "%d %b %Y %H:%M")
}
...
| ![](figures/glyphicons_135_inbox_out.png) \ **`r stats$n_contrib`** | ![](figures/glyphicons_043_group.png) \ **`r stats$n_team`** | ![](figures/glyphicons_045_calendar.png) \ **`r countdown(days_left)`** |
|:-------------:|:-------------:|:-------------:|
| contributions | participants | `r ifelse(days_left>0, "en cours", "terminé")` |
...
Le 4 janv. 2015 :
  ~ Note sur la prise en compte des coûts : [![](figures/glyphicons_181_download_alt.png)](https://www.dropbox.com/s/j12058t77w2qi4x/challenge.pdf?dl=1)
  
Le 3 nov. 2014 :
  ~ Les scores sont maintenant calculés sur l'ensemble du jeu test et non plus sur un sous ensemble. Le [classement][Classement] et l'[historique][Historique des contributions] ont été mis à jour en conséquence. La date limite des contributions a été repoussée au `r format(deadline, "%A %d %b %Y")`.
...
- **Date limite des contributions** : `r format(deadline, "%A %d %b %Y %H:%M", usetz=TRUE)`

# Description des données

Les données sont disponibles dans les deux fichiers suivant :

- Apprentissage : `data_train.RData` [![](figures/glyphicons_181_download_alt.png)](https://www.dropbox.com/s/5txejhqamm6bjzp/data_train.RData?dl=1)
- Test : `data_test.RData` [![](figures/glyphicons_181_download_alt.png)](https://www.dropbox.com/s/t6pt2d9kog173yv/data_test.RData?dl=1)
...
read_err = store_new_contrib("contrib", "history", deadline = deadline)
...
palette(brewer.pal(n = length(history)-1, name = "Dark2"))
...
Dernière mise à jour : ```r last_update()```
```{r, echo=FALSE, fig.cap="Historique des contributions - Nombre de contributions", fig.height=5, fig.width=9}
par(mar = c(5,4,4,7) + 0.1)
plot_contributions(history)
```
